...
if (!$nombre || mb_strlen($nombre) > 100) {
    ...
    echo json_encode(['error' => 'Nombre requerido y máximo 100 caracteres']);
    exit();
}
...
if (isset($_FILES['imagen_opcion']) && $_FILES['imagen_opcion']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    // Sanitizar nombre de archivo (solo por seguridad extra)
...
$stmt = $conn->prepare("INSERT INTO opcion_personalizacion (opc_nombre, opc_descripcion, opc_imagen) VALUES (?, ?, ?)");
if (!$stmt) {
    http_response_code(500);
    echo json_encode(['error' => 'Error de base de datos: ' . $conn->error]);
    exit();
}
$stmt->bind_param('sss', $nombre, $descripcion, $imagen_ruta);
$res = $stmt->execute();
$stmt->close();
$conn->close();
if ($res) {
    ...
    http_response_code(500);
    echo json_encode(['error' => 'Error al guardar opción']);
}

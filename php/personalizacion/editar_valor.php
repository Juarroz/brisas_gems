...
$stmt = $conn->prepare("UPDATE valor_personalizacion SET val_nombre=?, val_imagen=?, val_descripcion=? WHERE val_id=?");
if (!$stmt) {
    http_response_code(500);
    echo json_encode(['error' => 'Error de base de datos: ' . $conn->error]);
    exit();
}
$stmt->bind_param('sssi', $nombre, $imagen, $descripcion, $val_id);
$res = $stmt->execute();
$stmt->close();
$conn->close();
if ($res) {
    ...
    http_response_code(500);
    echo json_encode(['error' => 'Error al actualizar valor']);
}
